...
#  print(text_arr)
 ...
 def chat_funct(response_arr, query):
  ...
  context_statement = f'this is context for the prompt: {response_arr}'
  stream = ollama.chat(
   model='llama3.1',
   messages=[{'role':'user','content':context_statement}, {'role':'user','prompt': query}],
 ...
 chat_funct(response_arr=res,query=query)
